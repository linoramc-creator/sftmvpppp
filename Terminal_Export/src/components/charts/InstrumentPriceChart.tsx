...
// summary. Same visual language as the sidebar index sparklines (IndexSparkline):
// a thin line with a gradient area fill, blue when the month is up / red when
// down. Uses the daily close from the deterministic fundamentals endpoint
// (Yahoo), never the LLM.
...
// ── Line + gradient area chart (same pattern as IndexSparkline) ────────

function PriceAreaChart({ points, color, ticker }: { points: PricePoint[]; color: string; ticker: string }) {
  return (
  ...
  return (
    <div className="mb-4" style={{ background: "#0d1520", border: "1px solid #1e293b", borderRadius: 4, padding: "10px 12px 8px" }}>
      ...
      <div style={{ marginTop: 8 }}>
        <PriceAreaChart points={points} color={color} ticker={ticker} />
